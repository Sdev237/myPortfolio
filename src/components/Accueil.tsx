...
const ProfileSection = () => (
  <Row>
    ...
    <Row>
      <Col xs={12} sm={12} className="mt-md-5 my-lg-0 text-center text-lg-start">
        <UsefulLinks />
      </Col>
    </Row>
    <Row>
...
const AboutSection = ({ stacks }: { stacks: Stack[] }) => (
  <>
    ...
    <Col className="bio px-3 py-4">             
      <h1 className="mb-4 text-center text-lg-start" style={{opacity: '1'}}>Hi THERE👋HOW'S IT GOING</h1>
      <h2 className="text-center text-lg-start">Bienvenue Sur Mon Portfolio !</h2>
      <h4 className="text-center text-lg-start opacity-95">Ça vous dit de travailler ensemble sur des projets amusants ?</h4>
      <p className="text-center text-lg-start opacity-75" style={{opacity: '1'}}>
        Je suis passionné du développement web et des nouvelles technologies, j'ai la chance d'avoir pu développer mes compétences au sein de nombreuses entreprise a l'exemple de Orange Cameroun, HUGO Company LTD, CENADI et MODRY Sarl
      </p>

      <h6 className="mt-3">Stacks</h6>
      <Row>
        <Col className="stack py-4" style={{border: '1px solid #333', borderRadius: '0.4rem', marginLeft: '1rem'}}>
          <div className="d-flex flex-wrap justify-content-center ml-4">
            {stacks.map(({ icon, name }) => (
              <Button key={name} variant="outline-secondary" className="m-2 stackBtn">
